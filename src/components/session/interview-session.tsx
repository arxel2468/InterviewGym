...
export function InterviewSession({
  ...
  const { saveState, clearState, restoreMessages, isRestored } =
  ...
  const estimatedTotal =
    INTERVIEW_CONFIGS[interviewType as InterviewType]?.questionCount || 6
  ...
  const getAtmosphericMessage = () => {
    const msgs =
      ATMOSPHERIC_MESSAGES[difficulty] || ATMOSPHERIC_MESSAGES.standard
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      // Check if should end
      const candidateCount = updatedMessages.filter(
        (m) => m.role === 'candidate'
      ).length
      ...
      if (!interviewRes.ok || interviewData.success === false) {
        throw new Error(
          interviewData.friendlyMessage || 'Interviewer disconnected'
        )
      }
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <p className="text-sm capitalize text-zinc-400">{difficulty}</p>
            <span className="text-zinc-600">•</span>
            <p className="text-sm text-zinc-400">
              Question {candidateMessageCount + 1} of ~{estimatedTotal}
            </p>
          </div>
          <div className="mt-2 h-1 w-48 rounded-full bg-zinc-800">
            <div
      ...
      {/* Messages */}
      <Card className="flex flex-1 flex-col overflow-hidden border-zinc-800 bg-zinc-900/50">
        ...
        {/* Error */}
        {error && (
          <div className="border-t border-zinc-800 bg-red-500/10 p-4">
            <div className="flex items-center gap-3">
            ...
            {state === 'waiting_for_candidate' && !isRecording && (
              <>
                ...
                <Button
                  ...
                  disabled={!isOnline}
                  className="bg-gradient-primary h-14 px-8 hover:opacity-90"
                >
