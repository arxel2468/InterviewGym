...
const COMMON_GRAMMAR_ISSUES = [
  ...
  { pattern: /\brevert back\b/gi, suggestion: '"respond" or "get back to"', category: 'word_choice' },
  { pattern: /\bsame to same\b/gi, suggestion: '"exactly the same"', category: 'expression' },
  { pattern: /\bout of station\b/gi, suggestion: '"out of town" or "traveling"', category: 'expression' },
  { pattern: /\bpassed out from\b/gi, suggestion: '"graduated from"', category: 'word_choice' },
]
...
export function analyzeFluency(text: string): FluencyAnalysis {
  ...
  const advancedWords = [
    'implemented', 'architected', 'optimized', 'streamlined',
    'collaborated', 'spearheaded', 'leveraged', 'facilitated',
    'demonstrated', 'orchestrated', 'mitigated', 'scalable',
  ]
  ...
  const fillerWords = words.filter(w =>
    ['um', 'uh', 'like', 'basically', 'actually', 'so'].includes(w.toLowerCase())
  )
